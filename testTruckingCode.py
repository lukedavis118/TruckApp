...
# IF ERROR IS DUE TO SERVER BEING DOWN FOR OPENROUTESERVICE, SWITCH METHODS ---
    ...
    for row in firstLoads[:25]:
        cc += 1
        print('Processing {} out of {}'.format(cc,min(len(firstLoads),25)))
...
def findTripsExtended(inputs2,session,coords,row,data):
    ...
    if inputs2['startWithLoad']:
        ...
        itinerary.append({
            ...
            'originDeadhead': np.nan,
            'tripDistance': row['timeSegments'][-1]['distance'],
            'postedRate': np.nan,
            'rpm': np.nan,
            'accountName': np.nan,
            'daysToPay': np.nan,
            ...
            'endTime': inputs2['date_start'] + 
                        dt.timedelta(hours=row['nextDayTime']),
            'loadTime': np.nan,
